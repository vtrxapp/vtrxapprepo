...
function PricingScreen({ onContinue, onBack }) {
  ...
  const FREE_FEATURES = [
    "Personalised onboarding & fitness profile",
    "Daily mood check-in & workout adaptation",
    "3 workout plans",
    "Basic set & rep logging",
    "1 meal recommendation per day",
  ...
  const PREMIUM_FEATURES = [
    "Full workout library — unlimited exercises",
    "Unlimited meal swaps & full recipe library",
    "Weekly meal planner & grocery list",
    "Full AI post-workout analysis",
  ...
  return (
    <div style={{ position:"absolute",inset:0,background:BG,display:"flex",flexDirection:"column",overflowY:"auto" }}>
      ...
      <div style={{ padding:"52px 20px 0",flexShrink:0 }}>
        ...
        <div style={{ fontFamily:FONT,fontWeight:900,fontSize:22,color:"#fff",marginBottom:6 }}>Choose Your Plan</div>
        <div style={{ fontFamily:FONT,fontSize:13,color:"#888",marginBottom:24 }}>Start free. Upgrade when you're ready.</div>
      ...
      <div style={{ padding:"0 16px 40px",display:"flex",flexDirection:"column",gap:14 }}>
        ...
        <div style={{ background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.25)",borderRadius:14,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,marginBottom:4 }}>
          ...
          <div>
            <div style={{ fontFamily:FONT,fontWeight:800,fontSize:13,color:"#22C55E" }}>1 Month Free — No Card Needed</div>
            <div style={{ fontFamily:FONT,fontSize:11,color:"#888",marginTop:1 }}>Try everything Premium has to offer, completely free.</div>
          </div>
        </div>
        ...
        <button onClick={()=>{ if(selected==="premium") setIsPremium(true); onContinue(); }}
          ...
          {selected==="free" ? "Continue with Free" : "Start 1-Month Free Trial"}
        </button>
...
function NutritionHub({ onBack, energyKey, onLogout }) {
  ...
  const toggleSave = (id) => {
    if (!isPremium && !savedIds.includes(id) && savedIds.length >= 3) {
      setSaveMsg("Free plan: up to 3 saved recipes. Upgrade for unlimited.");
          ...
          <div>
            ...
            <div style={{ background:`${PRIMARY}12`,border:`1px solid ${PRIMARY}30`,borderRadius:16,padding:"14px 16px",marginBottom:14 }}>
              <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:6 }}>
                ...
                <span style={{ fontFamily:FONT,fontWeight:800,fontSize:13,color:"#fff" }}>{aiSug.title}</span>
              </div>
              <div style={{ fontFamily:FONT,fontSize:12,color:"#aaa",lineHeight:1.55,marginBottom:10 }}>{aiSug.tip}</div>
              <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
                {aiSug.rec.map(ri=>(
      ...
      <div style={{ padding:"0 16px" }}>
        ...
        {showMood&&!energyKey&&(
          <div style={{ background:CARD,borderRadius:20,padding:"16px",marginBottom:14,border:`1px solid ${BORDER}` }}>
            ...
            setShowMood(false);
            try {
              await apiCall("/ai/mood-recommendation", {
                method: "POST",
                body:   JSON.stringify({ mood: m.key }),
              });
            } catch {}
          }}
